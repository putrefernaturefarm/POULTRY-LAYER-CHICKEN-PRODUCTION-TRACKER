...
import NewVaccinationForm from './NewVaccinationForm'

export const metadata = { title: 'Log Vaccination | LayerPro' }
...
export default async function NewVaccinationPage() {
  ...
  return (
    <div className="p-4 md:p-6 max-w-xl mx-auto">
      <div className="mb-6">
        <h1 className="page-title">Log Vaccination</h1>
        <p className="text-sm mt-0.5" style={{ color: 'var(--ink-soft)' }}>{farm.name}</p>
      </div>
      <NewVaccinationForm flocks={flocks ?? []} houses={houses ?? []} />
    </div>
  )
}
